...
def calculate_IE(GS_un, GS_lab, TS_un, TS_lab, max_temp, min_temp, scale_factor, step_size, var):
    if GS_un == '' or GS_lab == '' or TS_un == '' or TS_lab == '':
        print "Did not finish selecting files!"
        return

    try:
        if float(min_temp) < 0 or float(max_temp) < 0 or float(scale_factor) < 0:
            print "Cannot have negative values for those parameters!"
            return
    except ValueError:
        print "Improper input! Should be a number"
        return

    print "--------------------------------------------------"
    print "Method", "\t", "Temp", "\t", "Isotope_effect", "\t", "IE with tunneling", "\n", "--------------------------------------------------"

    y_BM, x_BM = calc_IE_Bigeleisen(GS_un, GS_lab, TS_un, TS_lab, max_temp, min_temp, scale_factor, step_size)

    y_RR, x_RR = calc_IE_rigid_rotor_method(GS_un, GS_lab, TS_un, TS_lab, max_temp, min_temp, scale_factor, step_size)
